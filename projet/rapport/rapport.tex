...
\textit{One Piece} (\textit{Wan pîsu}) est une série animée japonaise (anime) adaptée
...
de la télévision mondiale. Sur la base de données IMDb, elle porte le code
...
Dans ce travail, une saga est qualifiée de \textbf{succès} si sa \textbf{note IMDb
...
\begin{enumerate}
  ...
  \item \textbf{Série temporelle} (Ch.~3 et 4)~: construction de la série des notes
        par saga et intégration numérique par trapèzes.
...
\label{tab:csv}
\begin{tabular}{L{5.5cm} r r L{5cm}}
\toprule
...
One Piece est présent dans \textbf{6 fichiers sur 8} (absent du fichier de films
et du top-250 épisode par épisode). La détection systématique des valeurs manquantes
...
Puisque le dataset ne découpe pas One Piece en saisons distinctes, nous utilisons
le découpage en \textbf{sagas narratives officielles} issu du wiki One Piece. Ce
choix est préférable à un découpage arbitraire (fenêtres de 50 épisodes) car il
correspond à des unités narratives réelles, défendables devant tout lecteur. Les
6 derniers épisodes du dataset (878--883, début de l'arc Reverie) sont fusionnés
avec la saga précédente pour constituer la \textit{Saga des Quatre Empereurs}.
...
Pour chaque saga, nous calculons une \textbf{note pondérée} à l'aide de la règle
des trapèzes. En traitant chaque épisode comme un point $t = 1, 2, \ldots, n$ et
sa note comme $R(t)$, la valeur moyenne de la fonction est~:

\begin{equation}
  \bar{R}_{\text{trap}} = \frac{1}{n-1} \int_{1}^{n} R(t)\,\mathrm{d}t
  \approx \frac{\Delta t}{n-1}\left[\frac{R_1}{2} + R_2 + \cdots + R_{n-1}
  + \frac{R_n}{2}\right]
  \label{eq:trapeze}
\end{equation}

avec $\Delta t = 1$. Contrairement à la moyenne arithmétique simple, cette formule
donne un demi-poids au premier et au dernier épisode de la saga. L'écart maximal
observé entre les deux méthodes est de \textbf{0{,}0105 point}, faible pour les
grandes sagas mais non nul. Le calcul se justifie pleinement dans les sagas à fort
démarrage suivi d'une chute abrupte (ou inversement), où la pondération temporelle
capture mieux la forme de la courbe.
...
function trapeze_mean(ratings::AbstractVector{<:Real})
    ...
    # integrale par la regle des trapezes (Delta t = 1)
    integral = ratings[1]/2 + sum(ratings[2:end-1]) + ratings[end]/2
    return integral / (n - 1)   # normalisation par la longueur de l'intervalle
end
\end{lstlisting}
...
La figure~\reff{fig:sagas} révèle une légère \textbf{tendance à la hausse} sur les
trois dernières sagas, ce qui motive la suite de l'analyse.
...
La règle des trapèzes, elle, leur attribue un \textit{demi-poids}~: cela revient
à dire que ces épisodes «~en bordure~» sont moins représentatifs de la tendance
de fond de la saga.

L'intérêt pratique apparaît sur la saga~2 (Alabasta)~: la saga démarre très fort
(épisode 62, note 8{,}2) avant de se stabiliser. La moyenne simple surpondère ce
pic d'ouverture~; la règle des trapèzes l'atténue, donnant une image plus fidèle
de la qualité \textit{durable} de la saga (écart observé~: $+0{,}003$ point).
...
épisodes consécutifs notés, en pondérant par la longueur $\Delta t$, ce qu'une
somme simple ne peut pas faire sans biais.
\end{tcolorbox}
...
La dynamique des notes à l'intérieur d'une saga est modélisée par l'équation
différentielle ordinaire (ODE) suivante~:
...
\begin{itemize}
  ...
  \item $\lambda > 0$ est la \textbf{vitesse de retour}~: un $\lambda$ grand
        signifie que les notes se stabilisent rapidement autour de $K$.
\end{itemize}
...
\textit{Ornstein-Uhlenbeck} (retour à la moyenne), couramment utilisé en finance
et en physique statistique.
...
\textbf{Objectif~:} comparer la distribution des notes de saga d'One Piece à celle
...
\begin{align*}
  H_0 &: \text{les distributions d'One Piece et du groupe de référence sont identiques} \\
...
\subsection{Bilan des chapitres mobilisés}

\begin{center}
\begin{tabular}{L{1.5cm} L{10.5cm}}
\toprule
\textbf{Ch.} & \textbf{Mobilisation dans ce projet} \\
\midrule
Ch.~2 & \code{Project.toml} + \code{Manifest.toml}, seed \code{42}, discussion
        erreur $\mathcal{O}(h^4)$ de RK4 vs $\mathcal{O}(h^2)$ d'Euler \\
Ch.~3 & Détection des \code{missing} avec \code{ismissing}, modélisation des
        notes par saga comme série temporelle, régression pour la prédiction de $K$ \\
Ch.~4 & ODE de retour à la moyenne $dR/dt = -\lambda(R-K)$, implémentation
        manuelle de RK4, intégration par trapèzes pour la note pondérée \\
Ch.~5 & Mann-Whitney (2 échantillons), Wilcoxon signed-rank (1 échantillon),
        bootstrap non-paramétrique — tous via \code{HypothesisTests.jl} \\
\bottomrule
\end{tabular}
\end{center}
...
Une intégration numérique par trapèzes (Ch.~4) calcule la note pondérée par saga.
...
Runge-Kutta 4, trapèzes, Mann-Whitney, Wilcoxon, bootstrap, prédiction.
\\[4pt]
