...
\subsection{La programmation scientifique}

La programmation scientifique consiste à utiliser le code comme outil de
résolution de problèmes réels~: modélisation, analyse de données, simulation
et prédiction. Elle se distingue du développement logiciel classique par son
orientation vers la rigueur mathématique et la reproductibilité des résultats.

Les langages de programmation dits «~généraux~» couramment utilisés sont~:
Fortran, C/C++, Java, Swift, C\#, Go, JavaScript et Python. Dans ce projet,
nous utilisons \textbf{Julia}, un langage conçu spécifiquement pour le calcul
scientifique~: il combine la simplicité de Python et les performances de C.

Le processus de travail typique du programmeur scientifique suit six étapes~:

\begin{enumerate}
  \item \textbf{Analyser le problème}~: comprendre la question posée et identifier
        les données disponibles.
  \item \textbf{Écrire le code}~: implémenter une première solution.
  \item \textbf{Analyser le comportement et les résultats}~: vérifier que les
        sorties sont cohérentes et interprétables.
  \item \textbf{Refactorer}~: réorganiser le code pour le rendre plus lisible
        et maintenable.
  \item \textbf{Itérer}~: affiner la solution en répétant les étapes précédentes.
  \item \textbf{Améliorer le modèle}~: enrichir l'approche avec des méthodes
        plus précises ou plus robustes.
\end{enumerate}

Avant de commencer à coder, j'ai donc commencé par \textbf{analyser le problème}~:
identifier une question de recherche concrète, choisir un jeu de données adapté,
et définir précisément ce que signifie le «~succès~» dans ce contexte. Ces décisions
...
\begin{tcolorbox}[justif, title={Pourquoi les notes IMDb forment-elles une série temporelle~?}]
Une \textbf{série temporelle} est une suite de valeurs numériques ordonnées dans
le temps, espacées de manière régulière. Les notes IMDb de One Piece satisfont
exactement cette définition~:

\begin{itemize}
  \item Chaque \textbf{épisode} constitue un point de mesure, dans l'ordre de
        diffusion chronologique.
  \item Les points sont \textbf{régulièrement espacés}~: un épisode par semaine
        en diffusion normale.
  \item La \textbf{granularité} peut être choisie~: épisode par épisode (883~points)
        ou saga par saga (9~points agrégés), selon le niveau d'analyse souhaité.
\end{itemize}

Dans ce projet, nous travaillons à la granularité \textbf{saga}~: chaque saga
devient un point de la série, caractérisé par sa note moyenne. C'est cette série
de 9~points qui servira d'entrée au modèle ODE pour prédire la saga~10.
\end{tcolorbox}
...
\begin{equation}
  ...
  \text{IC bootstrap à 95\,\%}~: \left[7{,}713\,;\, 7{,}933\right]
\end{equation}
...
ARIMA(1,0,0) & IC~95\,\% contient 8{,}0 & — & Oui (8{,}0 hors IC) \\
\bottomrule
\end{tabular}
\end{table}
...
\subsection{Test 4 : ADF et ARIMA (vérification de stationnarité + prédiction alternative)}

\subsubsection*{Test ADF — stationnarité de la série des épisodes}

Avant d'appliquer un modèle ARIMA, il est nécessaire de vérifier si la série
est \textbf{stationnaire}~: ses propriétés statistiques (moyenne, variance) ne
doivent pas dépendre du temps. On applique le \textbf{test de Dickey-Fuller
augmenté (ADF)} sur les 883~notes d'épisodes.

Le modèle ADF(1) avec constante est~:
\begin{equation}
  \Delta y_t = \alpha + \gamma\, y_{t-1} + \delta\,\Delta y_{t-1} + \varepsilon_t
\end{equation}
On teste $H_0 : \gamma = 0$ (racine unitaire, série non stationnaire)
contre $H_1 : \gamma < 0$ (série stationnaire).

\begin{table}[ht!]
\centering
\caption{Résultat du test ADF sur les 883 notes d'épisodes.}
\begin{tabular}{lr}
\toprule
t-statistique ADF & $-11{,}14$ \\
Valeur critique 1\,\% & $-3{,}43$ \\
Valeur critique 5\,\% & $-2{,}86$ \\
\midrule
\textbf{Conclusion} & Rejet de $H_0$ à 1\,\% \\
\bottomrule
\end{tabular}
\end{table}

La série est \textbf{stationnaire}~: $d = 0$, aucune différenciation n'est
nécessaire avant d'appliquer ARIMA.

\subsubsection*{ARIMA(1,0,0) — prédiction alternative de la saga~10}

On ajuste un modèle AR(1) sur les 9~notes moyennes de sagas~:
\begin{equation}
  y_t = \alpha + \varphi\, y_{t-1} + \varepsilon_t
  \quad \hat{\alpha} = 8{,}246,\quad \hat{\varphi} = -0{,}087
\end{equation}

La prédiction pour la saga~10 est~:
\begin{equation}
  \hat{R}_{10}^{\text{ARIMA}} = 7{,}565
  \quad \text{IC 95\,\%}~: \left[7{,}265\,;\, 7{,}866\right]
\end{equation}

Le seuil de 8{,}0 est \textbf{hors de l'IC à 95\,\%}, ce qui confirme
la conclusion du modèle ODE~: Wano ne devrait pas atteindre le seuil de succès.
Les deux modèles (ODE et ARIMA) s'accordent sur cette conclusion.
...
\begin{itemize}
  ...
  \item \textbf{Biais du groupe de référence~:} le groupe inclut des séries de
        ...
        homogène.
  \item \textbf{Saisonnalité et cycles non modélisés~:} une série temporelle
        peut présenter trois composantes~: une \textit{tendance} (couverte ici
        via la progression de $K$), une \textit{saisonnalité} (pics de notes à
        certains moments narratifs clés, comme Marineford ou Onigashima) et des
        \textit{cycles} (alternance d'arcs très notés et d'arcs plus faibles).
        Ces deux dernières composantes n'ont pas été modélisées~: avec seulement
        9~points de saga, le nombre d'observations est insuffisant pour estimer
        des effets saisonniers ou cycliques de manière fiable.
\end{itemize}
