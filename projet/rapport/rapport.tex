...
Chaque saga possède une note «~naturelle~» $K$ vers laquelle les épisodes tendent
à revenir. Si un épisode est très bien noté, le suivant redescend vers $K$~; s'il
est mal noté, le suivant remonte.
...
\textbf{Objectif~:} comparer la distribution des notes de saga d' da à celle
...
Ch.~4 & ODE de retour à la moyenne $dR/dt = -\lambda(R-K)$, implémentation
        manuelle de RK4, intégration par Simpson pour la note pondérée \\
